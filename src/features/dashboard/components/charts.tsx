...
export const Charts = ({ data, monthlyLimit = 0 }: ChartsProps) => {
  ...
  const categoryMap = useMemo(() => {
    return debts.reduce((acc: any, debt: any) => {
      const monthlyAmount = debt.amountPerMonth || (debt.totalAmount / (debt.totalInstallments || 1));
  ...
  // Lógica de Suscripciones vs Gasto Mensual (La sugerencia del usuario)
  const otherMonthlyExpenses = Math.max(0, totalToPayThisMonth - totalSubscriptions);
  const subPercentage = totalToPayThisMonth > 0 ? Math.round((totalSubscriptions / totalToPayThisMonth) * 100) : 0;

  const subVsMonthlyData = useMemo(() => ({
    labels: ['Suscripciones', 'Otros Gastos'],
    datasets: [{
      data: [totalSubscriptions, otherMonthlyExpenses],
      backgroundColor: ['#8b5cf6', 'rgba(255, 255, 255, 0.05)'],
      borderWidth: 0,
      cutout: '80%',
    }],
  }), [totalSubscriptions, otherMonthlyExpenses]);
  ...
  const healthPercentage = monthlyLimit > 0 ? (totalToPayThisMonth / monthlyLimit) * 100 : 0;
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div className="space-y-1">
              ...
              <p className="text-white/40 text-[10px] font-black uppercase tracking-[0.2em]">
                Uso del presupuesto: ${totalToPayThisMonth.toLocaleString()} de ${monthlyLimit.toLocaleString()}
              </p>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {/* SUSCRIPCIONES (NUEVO ENFOQUE DE PORCENTAJE) */}
        ...
        {/* CAPITAL PENDIENTE (EXCLUYENDO SUBS) */}
        <div className="bg-white/5 border border-white/10 rounded-[2.5rem] p-8 flex flex-col justify-between relative overflow-hidden group">
          ...
          <div className="mt-4">
             <div className="flex justify-between text-[9px] font-black text-white/20 uppercase mb-2">
                ...
                <span>{totalToPay > 0 ? Math.round((Number(totalToPayThisMonth - totalSubscriptions) / Number(totalToPay)) * 100) : 0}%</span>
             </div>
             <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden">
                <div 
                  className="h-full bg-indigo-500/50 transition-all duration-1000" 
                  style={{ width: `${totalToPay > 0 ? Math.min((Number(totalToPayThisMonth - totalSubscriptions) / totalToPay) * 100, 100) : 0}%` }} 
                />
             </div>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-12 items-center">
          ...
          <div className="w-full md:w-1/2 space-y-4">
            ...
            <div className="max-h-64 overflow-y-auto pr-2 space-y-3 custom-scrollbar">
              {(selectedCategory 
                ? debts.filter(d => d.category === selectedCategory)
                ...
                const value = isDebt ? item.amountPerMonth : item.value;
